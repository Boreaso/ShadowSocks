...
namespace Shadowsocks.View
{
    public class MenuViewController
    {
        ...
        private TrafficStatsForm trafficStatsForm;
        private TrafficStatsSuspensionForm trafficStatsSuspentionForm;
        private TrafficStats trafficStats;
        private string _urlToOpen;
        private string notifyIconText;
        private CancellationTokenSource cts;

        private long lastInboundCounter;
        private long lastOutboundCounter;
        private const int Max_CheckStats_Times = 5;
        ...
        private void UpdateTrafficDataStats()
        {
            while (true)
            {
                if (this.controller != null)
                {
                    ViewUtils.SetNotifyIconText(this._notifyIcon, this.notifyIconText +
                        $"\n[{I18N.GetString("Traffic Download")}]" + Utils.FormatBytes(this.controller.InboundCounter) +
                        $"  [{I18N.GetString("Traffic Upload")}]" + Utils.FormatBytes(this.controller.OutboundCounter));

                    this.trafficStats.CorrectStats(DateTime.Now);

                    long increment;
                    if ((increment = controller.InboundCounter - this.lastInboundCounter) > 0)
                    {
                        this.lastInboundCounter = controller.InboundCounter;
                        this.trafficStats.CurrentDownload += increment;
                        this.trafficStats.TodayDownload += increment;
                        this.trafficStats.MonthDownload += increment;
                        this.trafficStats.MonthUsed += increment;
                    }
                    if ((increment = controller.OutboundCounter - this.lastOutboundCounter) > 0)
                    {
                        this.lastOutboundCounter = controller.OutboundCounter;
                        this.trafficStats.CurrentUpload += increment;
                        this.trafficStats.TodayUpload += increment;
                        this.trafficStats.MonthUpload += increment;
                        this.trafficStats.MonthUsed += increment;
                    }
                }
        ...
        private void UpdateTrayIcon()
        {
            ...
            this.notifyIconText = I18N.GetString("Shadowsocks") + " " + UpdateChecker.Version + "\n" +
                          (enabled ?
                              I18N.GetString("System Proxy On: ") + (global ? I18N.GetString("Global") : I18N.GetString("PAC")) :
                              String.Format(I18N.GetString("Running: Port {0}"), config.localPort))  // this feedback is very important because they need to know Shadowsocks is running
                          + "\n" + serverInfo;
            ViewUtils.SetNotifyIconText(_notifyIcon, this.notifyIconText);
        }
        ...
        private void LoadMenu()
        {
            this.contextMenu1 = new ContextMenu(new MenuItem[] {
                ...
                new MenuItem("-"),
                this.statsItem = CreateMenuItem("Traffic Stats", new EventHandler(this.statsItem_Click)),
        ...
        private void controller_ConfigChanged(object sender, EventArgs e)
        {
            ...
            bool showStatsSuspentionForm = controller.GetCurrentConfiguration().showStatsSuspentionForm;
            if (this.statsItem.Checked != showStatsSuspentionForm)
            {
                this.statsItem.Checked = showStatsSuspentionForm;
            }
        }
        ...
        {
            statsItem.Checked = !statsItem.Checked;
            CheckStatsItem(statsItem.Checked);
        }

        private bool isFirstCheck = true;
        private void CheckStatsItem(bool isChecked)
        {
            if (isChecked)
            {
                this.CreateSuspentionForm();
            }
            else
            {
                this.DestorySuspentionForm();
            }
            ...
            controller.ToggleShowStatsSuspention(isChecked);
        }
